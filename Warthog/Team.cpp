...
void Team::removeChar(Character* ch1)
{
     for(aux2=0;aux2<aux;aux2++){


        if(character[aux2]==ch1){

            character[aux2]=character[aux-1];

            character[aux-1]=NULL;
...
void Team::removeChar(int a)
{
      character[a]=character[aux-1];
...
{
    std::string cor;
    if(color==0){cor=" Team:blue";}
    else if(color==1){cor=" Team:red"; }
    else if(color==2){cor=" Team:yellow";}
    else if(color==3){cor=" Team:white";}
    else{cor=" Team:black";}
    return nameS+cor;
}
